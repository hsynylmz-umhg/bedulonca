...
"""
Bedülonca V10.3.2 — Ürün Detay Sayfası (Kusursuz UI & Hizalama)
"""
...
#MainMenu, footer, [data-testid="stHeader"], [data-testid="stSidebar"], [data-testid="stSidebarNav"], [data-testid="collapsedControl"] { display: none !important; }
...
.navbar-logo-img { width: 80px; height: 80px; object-fit: contain; }
.navbar-brand { font-size: 22px; font-weight: 800; letter-spacing: -0.4px; }
.navbar-menu { display: flex; gap: 15px; align-items: center; }
.custom-nav-btn {
    display: inline-flex; align-items: center; gap: 8px; font-size: 14px; font-weight: 600;
    color: var(--text-color) !important; padding: 8px 14px; border-radius: 8px; transition: background 0.15s ease;
}
.custom-nav-btn:hover { background: rgba(128,128,128,0.1); opacity: 1 !important; }

/* SECTIONS & CARDS */
.section-label {
    font-size: 11px; font-weight: 700; letter-spacing: 1.6px; text-transform: uppercase;
    opacity: 0.5; margin-bottom: 10px; margin-top: 6px; border-bottom: 1px solid rgba(128,128,128,0.15);
    padding-bottom: 8px; display: flex; align-items: center; gap: 7px;
}
.section-label .fi { font-size: 14px; opacity: 0.8; }
.detail-header {
    border: 1px solid rgba(128,128,128,0.16); border-radius: 10px; padding: 16px 22px; margin-bottom: 20px;
...
/* ── KUSURSUZ METRİK HİZALAMA (DEĞER VE ZARAR KAPSÜLÜ YAN YANA) ── */
...
[data-testid="stMetricLabel"] {
    font-size: 10px !important; font-weight: 700 !important; text-transform: uppercase !important; letter-spacing: 0.8px !important; opacity: 0.55 !important;
}

/* Değer ve Kapsülü saran iç container'ı flex row (satır) yapıyoruz */
[data-testid="stMetric"] > div:nth-child(1) > div:nth-child(1) {
...
[data-testid="stMetricDelta"] { 
    ...
    margin: 0 !important; /* Kapsülün alt satıra itilmesini engeller */
}
...
/* TABS */
[data-testid="stTabs"] [data-baseweb="tab-list"] { background: transparent !important; border-bottom: 1px solid rgba(128,128,128,0.18) !important; }
[data-testid="stTabs"] [data-baseweb="tab"] { font-size: 12px !important; font-weight: 600 !important; opacity: 0.6; }
[data-testid="stTabs"] [aria-selected="true"] { background: rgba(37,99,235,0.09) !important; color: #2563EB !important; border-bottom: 2px solid #2563EB !important; opacity: 1 !important; }

/* ── BATCH (PARTİ) KARTLARI ŞIK TASARIM ── */
.batch-card { 
...
.batch-header {
    ...
    border-bottom: 1px dashed rgba(128,128,128,0.2); padding-bottom: 10px; margin-bottom: 10px;
}
.batch-badge {
    background: #2563EB; color: white; padding: 4px 8px; border-radius: 6px;
    font-size: 11px; font-weight: 700; letter-spacing: 0.5px;
}
.batch-date { font-size: 11px; opacity: 0.6; font-weight: 600; }
.batch-body { display: flex; gap: 30px; }
.batch-stat { display: flex; flex-direction: column; gap: 3px; }
.stat-label { font-size: 10px; text-transform: uppercase; font-weight: 700; opacity: 0.5; letter-spacing: 0.5px; }
.stat-val { font-size: 15px; font-weight: 700; }
.stat-val-sub { font-size: 12px; font-weight: 500; opacity: 0.6; margin-left: 4px; }

div[data-testid="stButton"] > button { font-weight: 600 !important; border-radius: 8px !important; font-size: 13px !important; }
</style>
...
# ── Yerel Animasyon Çözücüsü (Base64 Güvencesi) ──────────────────────────
def render_video_loader(filename: str, text: str) -> str:
    """Yerel webm dosyalarını base64'e çevirip HTML olarak döner, patlama riskini sıfırlar."""
    video_path = Path(_SCRIPT_DIR).parent / "assets" / filename
    if os.path.exists(video_path):
        with open(video_path, "rb") as f:
            video_b64 = base64.b64encode(f.read()).decode()
        video_src = f"data:video/webm;base64,{video_b64}"
    else:
        video_src = "" 
        
    return f"""
    <div style='display:flex; align-items:center; gap:10px; padding: 10px; margin-bottom: 15px; background: rgba(128,128,128,0.05); border-radius:8px; border: 1px solid rgba(128,128,128,0.1);'>
        <video width="40" height="40" autoplay loop muted playsinline>
            <source src="{video_src}" type="video/webm">
        </video>
        <span style='font-family: Inter, sans-serif; font-weight: 600; font-size: 13px; opacity: 0.8;'>{text}</span>
    </div>
    """
...
def fmt_tl(v: float) -> str: return f"₺{v:,.0f}".replace(",", ".")

def _sma(series: list, window: int) -> list:
    result = []
    for i in range(len(series)):
        if i < window - 1: result.append(None)
        else: result.append(sum(series[i - window + 1: i + 1]) / window)
    return result
...
def _build_system_prompt(product: dict, fifo_info: dict, usd_rate: float, intel_report: str) -> str:
    ...
    hist_str = " | ".join(f"{h['month']}: ₺{h['price']:,.0f} / {h['sales_qty']} adet / %{h['profit_margin_pct']}" for h in history) or "Tarihsel veri yok."
...
# Burada Kar Marjı (Değer) ve Zarar/Dikkat/Sağlıklı (Kapsül) yan yana çıkacak
met4.metric("Kâr Marjı", f"{margin_pct:.1f}%", delta="✅ Sağlıklı" if margin_pct >= 12 else "⚠️ Dikkat" if margin_pct >= 0 else "❌ Zarar", delta_color="normal" if margin_pct >= 12 else "inverse")
...
with col_charts:
    if not history:
        ...
        months, prices, sales_qtys, margins = [h["month"] for h in history], [h["price"] for h in history], [h["sales_qty"] for h in history], [h["profit_margin_pct"] for h in history]
        ...
        with tab1:
            ...
            fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.08, subplot_titles=("Satış Fiyatı (TL)", "Kâr Marjı (%)"), row_heights=[0.65, 0.35])
            fig.add_trace(go.Scatter(x=months, y=prices, mode="lines+markers", name="Fiyat", line=dict(color=COLOR_GREEN, width=2.5), marker=dict(size=7, color=COLOR_GREEN, line=dict(color="rgba(0,0,0,0.5)", width=1)), fill="tozeroy", fillcolor="rgba(22,163,74,0.1)", hovertemplate="<b>%{x}</b><br>₺%{y:,.0f}<extra></extra>"), row=1, col=1)
            marker_colors_m = [COLOR_RED if m < 0 else COLOR_YELLOW if m < 12 else COLOR_GREEN for m in margins]
            fig.add_trace(go.Scatter(x=months, y=margins, mode="lines+markers", name="Marj %", line=dict(color=COLOR_PURPLE, width=2), marker=dict(size=7, color=marker_colors_m, line=dict(color="rgba(0,0,0,0.5)", width=1)), fill="tozeroy", fillcolor="rgba(124,58,237,0.1)", hovertemplate="<b>%{x}</b><br>%{y:.1f}%<extra></extra>"), row=2, col=1)
            fig.add_hline(y=12, line_dash="dot", line_color=COLOR_YELLOW, opacity=0.5, row=2, col=1, annotation_text="Min. %12", annotation_font_color=COLOR_YELLOW, annotation_position="top right")
            fig.add_hline(y=0, line_dash="dash", line_color=COLOR_RED, opacity=0.4, row=2, col=1)
            fig.update_layout(paper_bgcolor=BG_PAPER, plot_bgcolor=BG_PLOT, font=dict(family="Inter"), margin=dict(l=10, r=10, t=50, b=10), height=420, showlegend=True, legend=dict(orientation="h", y=1.08, bgcolor="rgba(0,0,0,0)"), xaxis=dict(gridcolor=GRID_COLOR, rangeselector=dict(buttons=[dict(count=1, label="1A", step="month", stepmode="backward"), dict(count=3, label="3A", step="month", stepmode="backward"), dict(count=6, label="6A", step="month", stepmode="backward"), dict(step="all", label="Tümü")], bgcolor="rgba(128,128,128,0.1)", activecolor="rgba(37,99,235,0.2)", x=0, y=1.02)), xaxis2=dict(gridcolor=GRID_COLOR), yaxis=dict(gridcolor=GRID_COLOR, tickprefix="₺", tickformat=",.0f"), yaxis2=dict(gridcolor=GRID_COLOR, ticksuffix="%"))
        ...
        with tab2:
            ...
            fig2 = go.Figure()
            fig2.add_trace(go.Bar(x=months, y=sales_qtys, name="Satış Adedi", marker_color=bar_colors, opacity=0.80, text=sales_qtys, textposition="outside", hovertemplate="<b>%{x}</b><br>%{y} adet<extra></extra>"))
            fig2.add_trace(go.Scatter(x=months, y=sma2, mode="lines", name="SMA-2", line=dict(color=COLOR_YELLOW, width=2, dash="dot"), hovertemplate="SMA-2: %{y:.1f}<extra></extra>", connectgaps=True))
            fig2.add_trace(go.Scatter(x=months, y=sma3, mode="lines", name="SMA-3", line=dict(color=COLOR_PURPLE, width=2, dash="dash"), hovertemplate="SMA-3: %{y:.1f}<extra></extra>", connectgaps=True))
            fig2.update_layout(paper_bgcolor=BG_PAPER, plot_bgcolor=BG_PLOT, font=dict(family="Inter"), margin=dict(l=10, r=10, t=40, b=10), height=320, barmode="overlay", legend=dict(orientation="h", y=1.12, bgcolor="rgba(0,0,0,0)"), title=dict(text="Aylık Satış + SMA Trend Çizgileri", font=dict(size=13), x=0), xaxis=dict(gridcolor=GRID_COLOR), yaxis=dict(gridcolor=GRID_COLOR, ticksuffix=" adet"))
        ...
        with tab3:
            ...
            if len(prices) < 2: st.info("Fiyat esnekliği için en az 2 veri noktası gereklidir.")
            else:
                ...
                fig3 = go.Figure()
                fig3.add_trace(go.Scatter(x=prices, y=sales_qtys, mode="markers+text", name="Veri Noktaları", marker=dict(size=14, color=point_colors, opacity=0.85, line=dict(color="rgba(0,0,0,0.3)", width=1)), text=months, textposition="top center", textfont=dict(size=10), hovertemplate="<b>%{text}</b><br>Fiyat: ₺%{x:,.0f}<br>Satış: %{y} adet<extra></extra>"))
                sorted_pairs = sorted(zip(prices, trend_y), key=lambda t: t[0])
                fig3.add_trace(go.Scatter(x=[p[0] for p in sorted_pairs], y=[p[1] for p in sorted_pairs], mode="lines", name="Regresyon Trendi", line=dict(color=COLOR_BLUE, width=2, dash="dot"), hovertemplate="Trend: %{y:.1f}<extra></extra>"))
                ...
                fig3.update_layout(paper_bgcolor=BG_PAPER, plot_bgcolor=BG_PLOT, font=dict(family="Inter"), margin=dict(l=10, r=10, t=40, b=10), height=340, legend=dict(orientation="h", y=1.12, bgcolor="rgba(0,0,0,0)"), title=dict(text=f"Fiyat Esnekliği | Korelasyon: {corr:.2f}", font=dict(size=13), x=0), xaxis=dict(title="Satış Fiyatı (TL)", gridcolor=GRID_COLOR, tickprefix="₺", tickformat=",.0f"), yaxis=dict(title="Satış Adedi", gridcolor=GRID_COLOR))
    ...
    if not batches:
        ...
        for b in batches:
            ...
            qty = b.get('qty', 0)
            is_usd = product.get("buy_currency") == "USD"
            price_tl = price_usd * usd_rate if is_usd else price_usd
            
            # Dinamik para birimi gösterimi
            val_main = f"${price_usd:,.0f}" if is_usd else fmt_tl(price_tl)
            val_sub = f"<span class='stat-val-sub'>(≈ {fmt_tl(price_tl)})</span>" if is_usd else ""
            
            # Şıklaştırılmış Kart Yapısı
            st.markdown(f"""
            <div class="batch-card">
                <div class="batch-header">
                    <span class="batch-badge">PARTİ {b.get('batch_id','?')}</span>
                    <span class="batch-date">🗓️ {b.get('buy_date','?')}</span>
                </div>
                <div class="batch-body">
                    ...
                    <div class="batch-stat">
                        <span class="stat-label">Birim Fiyat</span>
                        <span class="stat-val">{val_main}{val_sub}</span>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        st.caption(f"Yöntem: `{fifo_info.get('fifo_method','?')}` · Ortalama FIFO Maliyet: **{fmt_tl(fifo_cost_tl)}**")
...
with col_chat:
    ...
    if intel_key not in st.session_state:
        ...
        intel_loader_ph.markdown(render_video_loader("web_intel.webm", "Canlı Web İstihbaratı taranıyor..."), unsafe_allow_html=True)
        ...
        if model is None:
            ...
            with chat_container:
                with st.chat_message("assistant"):
                    ...
                    ai_loader_ph.markdown(render_video_loader("ai_thinking.webm", "Bedülonca Gemini analiz ediyor..."), unsafe_allow_html=True)
